...
# 非交互式运行（自动进行 N 轮对话）
uv run mind --max-turns 20 --non-interactive
uv run mind "杂交物种形成" --max-turns 10 --non-interactive
```
...
| 参数 | 说明 |
|------|------|
| `topic` | 对话主题（位置参数，不指定则交互式输入） |
| `--max-turns N` | 最大对话轮数（用于非交互式模式） |
| `--non-interactive` | 非交互式模式（自动运行对话） |
| `--no-tools` | 禁用工具扩展能力（默认启用） |
| `--no-search` | 禁用网络搜索功能（默认启用） |
| `--tool-interval N` | 工具调用间隔（轮数），0 表示禁用自动调用 |
| `--test-tools` | 测试工具扩展功能 |
...
**1. AI 主动检索**
智能体可以根据对话需要，使用 `[搜索: 关键词]` 语法主动请求网络搜索：
```
智能体: 让我搜索一下相关资料 [搜索: 人工智能法律伦理]
系统: 🔍 正在搜索: 人工智能法律伦理
```

**2. 代码库分析**
智能体可以分析本地代码库，回答关于项目结构和代码的问题：
```
智能体: 让我查看一下项目中的相关实现
系统: 📂 正在分析代码库...
```
...
**配置检索功能：**
- 默认启用：`--no-tools` 和 `--no-search` 可以分别禁用
- 调用频率：通过 `--tool-interval N` 控制自动调用间隔（轮数）
- 环境变量：
  - `MIND_USE_SDK_TOOLS=false` 使用原始工具管理器
  - `MIND_ENABLE_MCP=true` 启用 MCP 服务器集成

### 对话控制

**AI 搜索请求**：智能体可使用 `[搜索: 关键词]` 语法主动请求网络搜索

**对话结束检测**：对话进行至少 10 轮后，AI 可通过 `<!-- END -->` 标记请求结束对话，系统会使用 AI 分析验证对话质量，并进入两轮过渡期自然收尾。
